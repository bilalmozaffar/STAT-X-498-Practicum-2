...
ggplot(dat_mini_1_impute, aes(x = price, y = y ,color = cut(age, breaks = c(18, 30, 55, 95),
    labels = c("College Age(18-25)", "Adults (30-55)", "Late life(Over 55)")))) +
geom_jitter(height = 0.1, width = 0.5) + 
  ...
  ggsave("price_rel_1_b.jpeg", width = 22, height = 7, dpi = 300)
```
...
# ggsave("poutcome_rel_1.jpeg", width = 22, height = 7, dpi = 300)
```
...
ggplot(dat_mini_1_impute, aes(x = factor(education, labels = c("Less than College", "College and More", "Unknown")),  group=y)) + 
    ...
    geom_text(aes( label = scales::percent(..prop..),
                   y= ..prop.. ), stat= "count", vjust = -.5) +
    labs(y = "Percent", fill="Education groups") +
    facet_grid(~y) +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
    scale_y_continuous(labels = scales::percent)+
...
# ggsave("housing_rel_1.jpeg", width = 22, height = 7, dpi = 300)
```
...
# ggsave("default_rel_1.jpeg", width = 22, height = 7, dpi = 300)
```
...
# ggsave("contact_rel_1.jpeg", width = 22, height = 7, dpi = 300)
```
